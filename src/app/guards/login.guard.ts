...
//Define el guard LoginGuard como un proveedor de servicios que se proporciona en el nivel de raíz de la aplicación.
@Injectable({
  providedIn: 'root'
})

//Define la clase LoginGuard.
export class LoginGuard {
  ...
  canActivate(): boolean{
    ...
    if(checkSession.user.id !== 0){ //Verifica si el usuario ya está autenticado (id !== 0).
      this.router.navigate(['']); //Si el usuario ya está autenticado, navega a la ruta raíz.
